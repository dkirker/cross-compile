...
@@ -1239,6 +1247,615 @@ osd_nav_toggle_announcer_new(struct navit *nav, struct osd_methods *meth, struct
...
+    double dCurrDist = transform_distance(projection_mg, &curr_coord, &cam_coord);
+    ret_attr = vehicle_get_attr(curr_vehicle,attr_position_speed,&speed_attr, NULL);
+    if(0==ret_attr) {
+      return;
+    }
+    double speed = *speed_attr.u.numd;
...
+    }
+  int idx = ((struct osd_speed_cam_entry*)qi_res->data)->cam_type;
+  if(idx>CAM_TRAFFIPAX) {
+    return;
+  }
+  int dir_idx = ((struct osd_speed_cam_entry*)qi_res->data)->cam_dir;
+  int dir = ((struct osd_speed_cam_entry*)qi_res->data)->direction;
+  int spd = ((struct osd_speed_cam_entry*)qi_res->data)->speed_limit;
+
+  char buffer [64+1]="";
+  char buffer2[64+1]="";
+  buffer [0] = 0;
...
+}
 struct osd_speed_warner {
 	struct osd_item item;
 	struct graphics_gc *red;
@@ -1252,7 +1869,6 @@ struct osd_speed_warner {
...
@@ -2484,6 +3100,7 @@ plugin_init(void)
